...
选择图片切图，在底部点击“标记重新生成”，再点击右上角“AI重新生成”。多选只标记其中未运行任务的图片；文字、背景和未分类不支持，隐藏的已标记图片仍会处理。

确认面板会显示当前设计图的任务、输入像素、可见直接子级数量和“设置 → 图片生成 / 修补”选用的 API / 模型。确认后按列表顺序逐张调用远程图片编辑接口，可能计费，不调用本地模型，也不自动重试。子级覆盖区会从参考图中清除并补全父图，子级仍独立保留。

生成结果先进入浅色审阅窗口，可以对比处理前 / 生成结果、缩放、拖动和 100% 检查；点击“应用此图”才替换切图，设计位置与尺寸不变。保留模型实际输出像素；任一边低于输入像素时禁止应用，不插值冒充高清。AI 会尽量保持主体、构图、颜色、文字与风格，但不保证像素或细节完全一致。

应用成功清除标记，支持工作区撤销；保存失败保留候选供重试。失败、取消和放弃结果不清除标记。候选仅存在于当前会话，关闭时确认放弃，刷新或切换工作区后不会保留；取消后远程服务仍可能已经计费。自动测试使用模拟响应，不产生远程生成费用。
